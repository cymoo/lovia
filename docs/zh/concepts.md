...
整个框架可以由五个概念撑起来。每个概念都对应一个真实会出问题的地方；本页会先
...
- **Transcript vs view**：transcript 是发生过什么的追加式记录；view 是某次
...
持久化又在另一个角落，最后没人说得清执行顺序。lovia 的回答是一条固定阶段的循环。
...
一个重要结果：**初始** agent 的姿态支配整个运行，包括 handoff 之后。转交只改变谁在
...
| `ToolError` | 工具以值得结构化的方式失败（通常由你抛出） |
...
- **Provider 是 protocol。** 两个内置适配器通过 `httpx` 调 OpenAI 和 Anthropic；
  第三个 provider 是一个 `Protocol` 实现，不是子类继承项目。
