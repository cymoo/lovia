...
`Scheduling(store)` 提供需要审批的 `schedule_run` Tool。模型可以建议创建定时任务，
但只有用户批准 Tool 调用后才会保存。`continue_session=True` 会把结果追加到同一对话；
否则每次触发都会创建新的 Session。每次触发至多执行一次；如果上一次运行尚未结束，
本次触发会直接跳过，不会排队。

重复任务可以携带停止条件（`until`，自然语言）：每次触发的运行在完成任务后会被要求
评估该条件，满足即取消任务——"每分钟检查日志，直到出现 ready"。确定性安全网
（`max_fires`、`expires_at`）保证即使条件一直不满足任务也会停止；工具设置 `until`
时必须搭配其一。插件还提供 `list_schedules` 和 `cancel_schedule`，二者无需审批：
取消只是停用（可在面板恢复或删除），而且条件满足时的自我取消发生在无人值守的
定时运行里，审批请求会被自动拒绝。
