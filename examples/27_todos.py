...
``todo_plugin()`` is the first :class:`~lovia.plugins.Plugin`. Attaching it adds a
...
from lovia import Agent, Runner, Todo, events, todo_plugin
...
async def main() -> None:
    ...
    async for event in Runner.stream(agent, task):
        if isinstance(event, events.ToolCallCompleted) and event.call.name == "todo_write":
            todos: list[Todo] = event.result  # structured list[Todo]
            print("\n— todo list —")
            for t in todos:
