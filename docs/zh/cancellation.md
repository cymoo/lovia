...
运行开始后仍可控制它：`CancelToken` 请求停止，`Mailbox` 在下一个 Turn 开始时加入新的
用户指令。

| 目标 | 通道 | 何时生效 |
| --- | --- | --- |
| 尽快停止当前 Run | `CancelToken` | 下一个安全点 |
| 补充或修正要求 | `Mailbox` | 下一个 Turn 开始时 |
...
安全点包括：每轮开始、每次 Tool 调用（执行前与结果落地后）以及重试退避期间。落在**最后一轮**
模型输出期间（直接作答、不再请求 Tool 的那一轮）的取消没有后续安全点，Run 会正常完成：这个
回答已经付过费，丢弃它只会让恢复时再付一次。
