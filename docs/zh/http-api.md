...
| `POST /api/chat/stream` | **SSE**：启动运行，或 attach 到 session 的 live run（注入新消息） |
| `POST /api/chat/reconnect?session_id=` | **SSE**：刷新后重新 attach，或恢复中断 checkpoint |
| `POST /api/chat/approve` | 处理 pending 审批：`{session_id, call_id, decision}` |
| `POST /api/chat/cancel?session_id=` | 停止 live run（保留已完成 turn） |
| `POST /api/chat/inject` / `uninject` | 为 live run 排队 / 撤回[追加指令](reliability.md#运行中追加指令) |
...
需要知道的语义：当某个 stream 拥有 session 时，`/api/chat` 返回 409；在 live session 上启动第二个
...
`event:` / `data:` 对，也就是 runner 的[类型化事件](streaming.md#事件目录)经过 JSON 编码后的形式：
...
reconnect 契约刻意简单：没有 Last-Event-Id 账本。客户端丢连接后（或 subscriber 队列溢出，服务端会关闭慢消费者）
只需重新 POST `/api/chat/reconnect`，就会收到新的权威 `snapshot`、当前 turn 在途事件回放
（包括仍 pending 的 `approval_required`），然后接 live tail。以 `:` 开头的 comment line 是 keep-alive，
跳过即可。
...
`ChatStore(session=..., meta_path=...)` 可以包住自定义 `Session` backend，同时保留 metadata 功能。
...
- **snapshot 按 turn，不按 token。** 重新 attach 时如果在句子中间，会从 turn buffer 重放这个句子的 delta；
  renderer 要能接受已经画过的 delta 再出现一次（按 turn 幂等渲染，或在 `snapshot` 时直接清空）。
