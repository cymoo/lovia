...
`lovia.testing` 提供了一个适合日常使用的测试替身：`ScriptedProvider`，一个真正的
...
`provider.calls[i]` 是第 *i* 轮输入的 chat 格式 view。它非常适合测试
...
- **在线冒烟测试**：打标，默认跳过，按需运行。本仓库使用 `pytest -m live_provider`，
...
- [评测](eval.md)：同一个测试替身，用来度量质量而不是接线
