...
lovia 不绑定默认供应商：你指定使用哪个模型，对应的 provider 会从环境变量读取凭证。
最短路径是：
...
结束。`handle.result()` 返回结果，或抛出运行期间的错误。完整事件清单见
[流式输出](streaming.md)。
...
- **[核心概念](concepts.md)**：花十分钟，后面的文档会轻松很多。这里解释一次运行
  真正做了什么，以及状态到底放在哪里。
