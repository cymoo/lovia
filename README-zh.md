...
或者一行命令启动完整的聊天 UI——自带记忆、技能、定时任务和当前目录的
工作区：
...
两个原语，底层都是普通工具。**Handoff** 移交对话——专家带着完整历史
接管，直接回答用户；**agent-as-tool** 委派有边界的子任务——子 agent
只看到提示词，答案作为工具结果返回：
...
manager = Agent(
    ...
    model="deepseek-v4-pro",
    tools=[summarizer.as_tool(description="Summarize a passage.")],  # 委派子任务
)
```
...
- **`Todo()`**——给模型一份多步任务的待办清单；当前清单每轮重新展示，
  却不增长转录。
- **`Skills(dir)`**——可复用的指令包（`SKILL.md` + 附属文件）：一行索引
  常驻提示词，完整内容按需加载。
- **`MCP(server)`**——接入 Model Context Protocol 服务器的工具，支持
  stdio 或 HTTP、按服务器加名称前缀和审批门禁。

自写插件只需一个 `name` 加一个返回贡献内容的异步 `setup()`。

→ [Plugins](./docs/en/plugins.md) · [Skills](./docs/en/skills.md) · [MCP](./docs/en/mcp.md)

### 记忆

跨会话的长期记忆——常驻提示词的 **Notes** 加可检索的 **Archive**，
零配置，一次加一个参数即可升级：
...
自带页面是可选的：全部功能都通过 JSON + SSE 的 REST API 暴露（`/api/docs`
可浏览），用 `create_app(agent, ui=False)`——或把 router 挂进你自己的
FastAPI 应用——即可在同一套端点上自定义前端。
