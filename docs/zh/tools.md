...
如果你想有意识地塑造这条错误消息，可以抛 `ToolError`（可带 `hint=`）。
...
- [Handoff](multi-agent.md) 工具永远是屏障，不管 flag 怎么设。这保证“同一 turn 中
  第一个 handoff 胜出”没有竞态。内置工作区 mutator（`write_file`、`edit_file`、
  `shell`）默认 `parallel=False`；只读工具保持并发。
- Preflight，也就是预算检查、审批、参数校验，总是按请求顺序串行执行，所以审批提示
  和预算计数在并发执行时仍然确定。
...
（默认 **200,000 字符**，是防止失控 payload 的保险线，不是策略）。更长的输出会保留
...
渲染器。renderer 只处理**成功**结果；runner 生成的 `"Tool error: ..."` 字符串会绕过
...
- **截断上限按渲染结果的字符数计算。** 200k 字符约等于 50k tokens。如果你的工具确实
  会合法返回更多内容，请提高上限或把 payload 持久化到别处；静默丢中间内容更糟。
