...
`LOVIA_MODEL` 决定所有示例使用的模型,例如 `openai:gpt-5.5` 或
`anthropic:claude-4-8-opus`。把 `OPENAI_BASE_URL` 指向任意 OpenAI 兼容服务
(DeepSeek、Ollama、vLLM 等),即可继续使用 `openai:<model>` 形式的字符串。
