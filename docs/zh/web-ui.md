...
对话历史、编辑后重发、重新生成、标题、审批、定时任务、记忆编辑、上下文用量显示，以及
只读的 Workspace 文件面板。
...
正在运行的会话会在侧栏显示脉动圆点，也可以直接从侧栏停止。UI 会定期检查后台状态：
未打开的会话在后台运行结束时，页面会弹出提示；如果标签页处于后台，浏览器标签页标题中
还会显示未读数。
定时任务会记录最近一次执行结果，并在面板中显示 ✓ 或 ✕；将鼠标停在失败标记上即可查看
错误信息。
