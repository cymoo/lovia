...
Session 让 agent 在**一段对话内**拥有历史；不同对话之间没有东西会自动传递，用户每次都得
重新解释偏好。`Memory` 插件提供长期记忆，它由模型已经理解的**两层、三个动词**组成：
...
这就是完整的零配置设置：标准库 SQLite 全文搜索，不需要服务，不需要 embedding。
召回效果仍然不错，因为 LLM 会弥合词面差异（见下）。
...
└── vectors.db     # 冷层：向量检索臂（只有 embedder= 时存在）
```

`MEMORY.md` 刻意保持普通 markdown：用编辑器打开，删一行，就完成了。写入是原子的；
读取时非 bullet 行会被忽略。
...
Notes 会保持在 `notes_budget` 内（默认 2000 字符，模型会看到一个 meter）。digest 后如果超预算，
会再做一次 consolidation 调用，把列表合并重写到预算内。

默认 curation **内联**运行：`Runner.run` 返回时，memory 已经整理完。长生命周期宿主可以传
`curate_in_background=True`，这样运行的最终事件不会被 curation 的模型调用拖住；关闭时 await
`mem.drain()`，把在途整理收尾（内置 web server 正是这样做的，带 15s 上限）。
...
**零配置**是 SQLite FTS5：在 CJK-aware bigram 索引上做 bm25。LLM 会补足关键词检索错过的东西：
`recall` 查询会在搜索前扩展同义词和翻译（`expand_query="auto"` 只在默认纯词面索引时开启），
命中结果会作为模型写出的 summary 返回，而不是裸摘录（`summarize_recall=True`）。这两个 LLM
辅助都是 fail-open：扩展或总结出错时，会退化到原始 query / 原始命中。
...
**`index=`** 完全替换检索层。`Index` 是围绕普通 docs 的三个方法：`add` / `remove` /
...
检索臂可以用 `|` 组合：`KeywordIndex(...) | VectorIndex(...) | my_arm` 会得到一个
RRF 融合的 `HybridIndex`。读取 fail-open（坏掉的臂会跳过），写入发往每个臂；任何 index
只要 mix in `Fusable` 就能获得 `|` 运算符。（`embedder=` 和 `index=` 互斥；embedder 只是构建
默认 hybrid 的语法糖。）

热层同样可替换：`NotesStore` 只有两个方法（`load`/`save` fact list）。规范化、去重和预算策略
都留在插件里，所以 Redis 或 DB backed store 只需十几行。`FileNotesStore`，也就是
`MEMORY.md` 写入器，是参考实现。
...
| 字段 | 默认值 | 作用 |
| --- | --- | --- |
| `root` | `./.lovia/memory` | 默认存储位置（显式传入某层 backend 时，该层忽略它） |
| `notes` | `None` → `MEMORY.md` 文件存储 | 热层 backend |
| `index` | 默认关键词索引 | 冷层 backend；`None` 关闭这一层和 `recall` 工具 |
| `embedder` | `None` | 给默认索引加向量臂 |
...
| `notes_budget` | `2000` | Notes 字符预算，也是 prompt meter 和 consolidation 触发点 |
| `model` | 宿主 agent 的模型 | curation/recall 辅助查询使用的模型 |

辅助查询（digest、consolidation、query expansion、summarization）会 dogfood `Runner.run`：
用一个没有工具、没有插件、temperature 0 的子 agent，因此复用你的 provider 链，且**不会递归**
（子 agent 没有 Memory 插件）。因为 lovia 的 transcript 是持久的，而且
[压缩只作用于 view](context.md)，digest 会在**完整** transcript 上跑一次：它是整理，不是抢救。

## 容易踩的点

- **自定义 backend 被每个运行共享**，可能并发访问。它们必须并发安全，插件也不会关闭它们；
...
- **后台 curation 是 best-effort。** 进程不 `drain()` 就退出，可能丢失最后一次运行的整理结果。
  这是有意接受的折中（transcript 仍在 session 里），但如果你期待强持久性，会比较意外。
- **`recall` 只和已经归档的内容一样好。** 之前用 `index=None` 的对话，后来就是搜不到；
  archive 从你开启它那一刻才开始。
...
- [Web UI 与服务端](web.md)：内置 Memory 编辑器
