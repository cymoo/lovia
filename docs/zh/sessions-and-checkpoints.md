...
它是跨运行的多轮记忆。**checkpoint** 回答“这次运行走到哪里了？”；它用于单次运行内的
...
`session_id` 由你控制，可以按用户、线程、工单，或产品里对“对话”的叫法来定。runner 会在运行
...
- [核心概念：Session vs checkpoint](concepts.md#session-vs-checkpoint)：关键区别
