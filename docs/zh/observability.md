...
[流式输出](streaming.md)产出的类型完全相同，所以即使没人消费 stream，instrumentation 仍然工作。
...
- **Fail-open**：handler 抛异常会被记录（warning，带 traceback）并跳过。坏掉的指标不应中止被观察的运行。
...
内置三个实现：`NoopTracer`（默认，让 instrumentation 没有成本）、`ConsoleTracer`
...
lovia 在 `lovia` logger 下记录结构化叙事日志：`run.start`、
...
- [流式输出](streaming.md)：hooks 接收的完整事件目录
