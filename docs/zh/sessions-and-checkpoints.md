...
- **按 `run_id` 幂等。** 对已保存的 `run_id` 再次 append 是 no-op，所以重放运行不会重复写入 entries。
...
必须熬过崩溃或可以安全重发的运行，需要加 checkpoint：
...
`RunSnapshot` 保存本次运行自己的 entries，加一个小的可变 head（`RunHead`）：活跃 agent 名、
...
**drain pending tool calls**。也就是说，崩溃时留下但没有结果的工具调用会重新执行（同一个 turn
number），之后循环继续。已完成结果不会重跑；悬空调用会重跑。带副作用的工具请按 at-least-once
设计，或让它们幂等。
...
- **`InMemory*` 存储无法跨进程存活。** 它们用于测试和临时聊天；把
  `InMemoryCheckpointer` 和“崩溃恢复”配在一起是类别错误。
