...
可复用能力通常一次需要好几样东西：一个工具、一段解释它的 prompt、每轮提醒，可能还要
hook 和 teardown。很多框架会让你把它们分别接进不同注册表。lovia 的 **plugin** 是一个
对象，能一次贡献这些内容，而且它是框架的**唯一**扩展轴：Skills、MCP、Todo 和 Memory
都基于同一机制。
...
插件是任何拥有 `name` 和异步 `setup()`、并返回 `PluginInstance` 的对象：
...
runner 会在**每次运行** await 一次 `setup()`；如果 [handoff](multi-agent.md) 到了另一个
agent，也会对那个 agent 的插件各跑一次。返回的贡献内容会合并到运行循环的固定槽位：
...
| `aclose` | 运行结束时 await 的 coroutine（多个插件按 LIFO，best effort） |
...
这是比较特别的槽位。`ViewInjector` 每轮都会接收实时 `RunContext`，并返回要追加到**本轮模型
...
因为注入的 entries 不进入 transcript 或 session，它们不会随着 turn 增长而累积，不会破坏
provider 的缓存 prompt 前缀，恢复运行时也不会回放它们。injector 应该每轮重新生成自己的
内容（提醒、时钟、todo list）。injector 是 **fail-open** 的：抛异常会记录日志并跳过，
...
一个有状态插件的完整形状：共享 backend、一个工具、提示词文本，一屏代码：
...
injector，把当前列表每轮以 `<system-reminder>` block 重新展示。它给模型保持计划更新的可见
压力，但 transcript 零增长。每个 item 包含 `content`、`TodoStatus`
...
- **`setup()` 按 agent、按运行执行，包括 handoff 目标。** 同一个插件如果挂在 handoff
  两侧，会在一次运行中激活两次；请让 `setup()` 便宜，并且效果上幂等。
- **instructions 每次运行静态。** `PluginInstance.instructions` 只在运行开始时渲染一次；
