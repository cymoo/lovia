...
实际效果：一个表里没有的模型，代价是**一次**撞墙，仅此一次。学到的窗口会按
`(endpoint, model)` 在进程内记忆，同时持久化进 session，之后的每一轮（以及之后的每次 run）
都按真实值计算。

两点值得知道：

- **`/models` 探测在帮不上忙时一律跳过**：你已经显式配置了窗口、端点已知什么都不公布
  （`api.openai.com`、`api.anthropic.com`）、或者已经问过了。结果（包括 miss）按
