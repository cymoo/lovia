...
只在提示词里要求“返回 JSON”并不可靠。设置 `output_type` 后，lovia 会按声明的类型解析并
校验最终答案；失败时按配置尝试修复，仍不合格则明确报错。

如果输出只给人阅读，保持默认 `str` 即可；只有下游代码需要稳定字段、类型或约束时，
才设置 `output_type`。外部副作用不应放在输出校验里，应通过 Tool 执行并单独审批。
