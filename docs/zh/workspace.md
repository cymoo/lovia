...
对可写工作区，该片段还会给出布局约定：草稿和中间产物放进 `tmp/`（视为可随时清理），没人要的文件
一律不写；交付物的位置则跟随根目录本身的形态——在 git 仓库里遵循仓库自己的布局与规范（且永不提交
`tmp/`），其他情况沿既有目录结构扩展、新任务产出多个相关文件时建主题子目录，而不是散落在根目录。
`lovia web` 的文件面板从另一侧配合这一约定：`tmp/` 与环境垃圾一同隐藏，Recent 只展示交付物。
