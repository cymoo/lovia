...
Agent 代码也应该有离线、免费、确定性的测试。需要网络、会抖、每次都花钱的“测试”，最后通常没人会跑。
`lovia.testing` 提供了一个适合日常使用的测试替身：`ScriptedProvider`。它是一个真正的
...
script 描述的是模型这一侧的对话：每个条目按顺序回应一次模型调用。实际工具仍会执行，只有 LLM 被脚本替代；
所以测试覆盖的是**实际运行循环**：schema 校验、并发执行、审批门禁、结构化输出解析、session 持久化。
...
| `batch(("a", {...}), ("b", {...}))` | 同一轮请求多个调用，用来测试[并发执行](tools.md#并发执行与屏障) |
...
[动态 instructions](agents.md#instructions)、[view injectors](plugins.md#view-injector每轮临时注入)，以及
...
- [评测](eval.md)：同一个测试替身，用来度量质量，而不是验证接入是否正确
- [Provider](providers.md#自定义-provider)：`ScriptedProvider` 也是一个可参考的 `Provider` 实现
