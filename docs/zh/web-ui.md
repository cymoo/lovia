...
工具打开它——这对任何模型都有效，上传的文件也会出现在文件面板中。点击附件不会离开应用：
文件在文件面板中打开，图片在灯箱（lightbox）中打开，而不是新开标签页或触发下载。
...
上传大小上限由 `LOVIA_MAX_UPLOAD_MB` 控制（默认 25 MiB），文件类型限制为内置的常见图片、
文档、数据与代码扩展名白名单。可用 `LOVIA_UPLOAD_ALLOWED_EXT`（逗号/空格分隔的扩展名，或
`*` 表示不限）覆盖。
